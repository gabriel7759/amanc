...
class Controller_Backend_Manage_Featured extends Controller_Backend_Template {
	
	public function before()
	{
		parent::before();
		
		$this->model  = new Model_Featured;
	}
	
	public function action_index()
	{
		$page     = Arr::get($_GET, 'page', 1);
		$order_by = Arr::get($_GET, 'order_by', 'featured.position');
		...
		$this->template->content->type_id    = $type_id;
	...
	public function action_form()
	{
		...
		{	
			...
			if ( ! $valid->check())
			{
				$this->template->errors = $valid->errors('featured');
			}
			else
			{	
				if ($data['id'])
