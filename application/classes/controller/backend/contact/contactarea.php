...
class Controller_Backend_Contact_Contactarea extends Controller_Backend_Template {
	
	public function before()
	{
		parent::before();
		
		$this->model  = new Model_Contactarea;
	}
	
	public function action_index()
	{
		$page     = Arr::get($_GET, 'page', 1);
		$order_by = Arr::get($_GET, 'order_by', 'contactarea.position');
	...
	public function action_form()
	{
		...
		{	
			...
			if ( ! $valid->check())
			{
				$this->template->errors = $valid->errors('contactarea');
			}
			else
			{	
				if ($data['id'])
